 ...
 */
public class PixelTest_ implements PlugInFilter {
	ImagePlus imp;
	...
	public void run(ImageProcessor ip) {
		...
		for (int y= 0; y<h; y++) {
			...
			for (int x=0; x<w; x++) {
				...
				int pixel = srcpix[srcline + x];
				int red = (pixel >> 16) & 0xFF;
				int green = (pixel >> 8) & 0xFF;
				int blue = pixel & 0xFF;
				final int thresh = 50;
				int destpixel = (int)((red+green+blue)/3);
				dstpix[dstline + x] = (destpixel > thresh) ? 0xFFFFFF : 0x000000;
			}
		}
